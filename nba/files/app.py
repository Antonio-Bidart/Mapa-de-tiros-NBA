...
with st.sidebar:
    ...
    # Checkboxes con tiempo estimado — el usuario sabe qué va a esperar
    quiere_video  = st.checkbox("🎬 Animación por partidos", value=False,
                                help="~20 segundos. Muestra los tiros acumulándose partido a partido.")
...
if buscar:
    ...
    # Generamos solo lo que el usuario eligió
    resultados = {}
...
if "resultados" in st.session_state and st.session_state.resultados:
    ...
    with col_grafico:
        ...
        # Construimos los tabs dinámicamente según lo que se generó
        labels = []
        ...
        if "video" in resultados:
            with tabs[tab_idx]:
                if not st.session_state.video_mostrado:
                    with open(resultados["video"]["ruta"], "rb") as f:
                        st.video(f.read(), autoplay=True)
                    time.sleep(resultados["video"]["duracion"])
                    st.session_state.video_mostrado = True
                    st.rerun()
                else:
                    st.info("La animación ya se reprodujo. Presioná Generar para verla de nuevo.")
            tab_idx += 1
